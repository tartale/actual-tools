// Vendored from actualbudget/actual (MIT licensed), commit 353e5dd26aa98503b2e88b9b25004c3ea5eeef7d:
//   packages/desktop-client/src/components/reports/reports/monte-carlo/monteCarloSimulation.ts
//   (blob 3bbd8d45a64e74316f9b02bac9e2542f6a0fb632)
// Copyright James Long and Actual Budget contributors, MIT licensed -- see LICENSE-ACTUAL.txt in
// this directory, or https://github.com/actualbudget/actual/blob/master/LICENSE.txt.
//
// Run in-process instead of only configuring Actual's own monte-carlo-card widget and viewing the
// result there -- see .claude/memory/monte-carlo-vendoring.md for why, and `./actual vendor check`
// for the drift check against the commit pinned above. Kept as close to a verbatim copy as
// possible (including the runDetail/captureRunDetail drill-in machinery this app doesn't wire up
// yet) so that check's diff reflects real upstream changes, not our own edits tangled in with
// them. Original style (single quotes, semicolons) left as-is for the same reason. The only
// changes from upstream:
// - MAX_SAFE_NUMBER (a trivial constant from @actual-app/core/shared/util) inlined directly.
// - The *FromMeta functions and monteCarloConfigFromMeta now take this app's own
//   MonteCarloPotMeta/MonteCarloSpendingPhaseMeta/MonteCarloWithdrawalRuleMeta/
//   MonteCarloTaxBandMeta/MonteCarloContributionMeta/MonteCarloCardMeta (fire-dashboard.ts)
//   instead of importing Actual's own -- those were already modeled to mirror Actual's wire
//   format exactly (see that file's own doc comments), so the functions work unchanged.
// - MonteCarloAllocationPreset is defined locally below with the full upstream union (including
//   'custom'/'custom-mix', which this app's own narrower fire-accounts.ts type of the same name
//   excludes) since the engine's own internal pot type needs the wider union, even though nothing
//   this app generates ever supplies those two values.
// - MonteCarloReturnModel/MonteCarloTaxModel/MonteCarloWithdrawalStrategy/MonteCarloTaxBandMeta/
//   MonteCarloWithdrawalRuleMeta import from fire-accounts.ts instead of Actual's own
//   types/models -- exact matches, not narrowed.

import { HISTORICAL_ANNUAL_RETURNS } from './monte-carlo-historical-returns.ts';
import type { HistoricalAnnualReturn } from './monte-carlo-historical-returns.ts';
import type { MonteCarloReturnModel, MonteCarloTaxBandMeta, MonteCarloTaxModel, MonteCarloWithdrawalRuleMeta, MonteCarloWithdrawalStrategy } from '../../fire-accounts.ts';
import type { MonteCarloCardMeta, MonteCarloContributionMeta, MonteCarloPotMeta, MonteCarloSpendingPhaseMeta } from '../../fire-dashboard.ts';

/** Mirrors Actual's own MonteCarloAllocationPreset (types/models) -- defined locally since this
 * app's own fire-accounts.ts type of the same name deliberately excludes 'custom'/'custom-mix'
 * (this app never generates either), but the engine's own pot type needs the full union. */
...
const MAX_SAFE_NUMBER = 2 ** 51 - 1; // packages/loot-core/src/shared/util.ts in actualbudget/actual
...
export function monteCarloConfigFromMeta(
  meta: MonteCarloCardMeta | undefined,
): MonteCarloConfig {
  return {
